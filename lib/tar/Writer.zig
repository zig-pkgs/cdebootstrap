...
fn writeHeader(
    ...
    if (options.mode != 0) try header.setMode(options.mode);
    if (options.uid) |uid| try header.setUid(uid);
    if (options.gid) |gid| try header.setGid(gid);
...
pub const Header = extern struct {
    ...
    pub fn setUid(w: *Header, uid: u32) !void {
        try octal(&w.uid, uid);
    }

    pub fn setGid(w: *Header, gid: u32) !void {
        try octal(&w.gid, gid);
    }
